...
# Chaque accompagnateur a son code unique comme demandé
ACCES = {
    "admin": "admin77",
...
# --- 3. GÉNÉRATEUR PDF (MISE EN FORME MIROIR EXCEL) ---
...
def generate_pdf(data, title_prefix="Rapport"):
    ...
    draw_section("1.Formule : Achat de matière premières", ["Dép.", "Trait.", "Val.", "AR", "Fin.", "Reçus", "Mnt"], ["MP_D", "MP_T", "MP_V", "MP_A", "MP_F", "MP_R", "MP_M"])
    draw_section("2.Formule : Triangulaire", ["Dép.", "Val.", "TBq", "Not.", "AR", "Fin.", "10%", "90%", "PV.E", "PV.D", "Rec.", "Mnt"], ["TR_D", "TR_V", "TR_B", "TR_N", "TR_A", "TR_F", "TR_1", "TR_9", "TR_E", "TR_D", "TR_R", "TR_M"])
    draw_section("5.Algérie Télécom", ["Dép.", "Val.", "TBq", "Not.", "10%", "90%", "PV.E", "PV.D", "Rec", "Mnt"], ["AT_D", "AT_V", "AT_B", "AT_N", "AT_1", "AT_9", "AT_E", "AT_D", "AT_R", "AT_M"])
    draw_section("6.Recyclage", ["Dép.", "Val.", "TBq", "Not.", "10%", "90%", "PV.E", "PV.D", "Rec", "Mnt"], ["RE_D", "RE_V", "RE_B", "RE_N", "RE_1", "RE_9", "RE_E", "RE_D", "RE_R", "RE_M"])
    draw_section("7.Tricycle", ["Dép.", "Val.", "TBq", "Not.", "10%", "90%", "PV.E", "PV.D", "Rec", "Mnt"], ["TC_D", "TC_V", "TC_B", "TC_N", "TC_1", "TC_9", "TC_E", "TC_D", "TC_R", "TC_M"])
    draw_section("8.Auto-entrepreneur", ["Dép.", "Tra.", "Val.", "TBq", "Not.", "AR", "Fin.", "10%", "90%", "PV.E", "PV.D", "Rec", "Mnt"], ["AE_D", "AE_T", "AE_V", "AE_B", "AE_N", "AE_A", "AE_F", "AE_1", "AE_9", "AE_E", "AE_D", "AE_R", "AE_M"])
...
if st.session_state.role == "Administrateur":
    ...
    t1, t2, t3 = st.tabs(["Base de Données", "Téléchargements", "Codes d'accès"])
    ...
    with t2:
        if not df.empty:
            ...
            if st.button("Générer le TOTAL Agence"):
                ...
                total_data.update({'Accompagnateur': "TOTAL AGENCE", 'Mois': m_sel, 'Annee': 2026})
                st.download_button("📥 TÉLÉCHARGER LE CUMUL", generate_pdf(total_data, "TOTAL"), f"Total_{m_sel}.pdf")
    with t3: st.table(pd.DataFrame(list(ACCES.items()), columns=["Nom", "Code Personnel"]))
    if st.button("Déconnexion"): st.session_state.auth = False; st.rerun()
    st.stop()

# --- 6. FORMULAIRE ACCOMPAGNATEUR (86 CASES) ---
st.title(f"Saisie Bilan : {st.session_state.user}")
...
def ui_sec(label, p, kp):
    ...
    data[f"{p}_N"] = c4.number_input(f"Notif. Banque", key=f"{kp}4")
    ...
    data[f"{p}_E"] = c9.number_input(f"PV Existence", key=f"{kp}9")
    c10, c11, c12 = st.columns(3)
    data[f"{p}_D"] = c10.number_input(f"PV Démarrage", key=f"{kp}10")
    data[f"{p}_R"] = c11.number_input(f"Reçus", key=f"{kp}11")
    data[f"{p}_M"] = c12.number_input(f"Montant Remb.", key=f"{kp}12")
