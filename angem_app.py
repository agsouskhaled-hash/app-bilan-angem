...
def save_to_gsheet(data_dict):
    try:
        ...
        try:
            ...
            # Création de l'onglet avec assez de colonnes pour les 84 cases
...
# --- 3. GÉNÉRATEUR PDF (MODÈLE FIDÈLE À L'IMAGE) ---
...
def generate_pdf_bytes(data):
    ...
    def draw_pdf_table(title, prefix, headers):
        pdf.set_fill_color(255, 230, 204) # Couleur Beige
    ...
    # CORRECTION CRITIQUE : Conversion en octets bruts pour Streamlit
    return pdf.output()
...
def build_rubrique(label, p, extension=False):
    st.subheader(label)
    cols = st.columns(5)
...
# Onglets
tabs = st.tabs(["Matière Première", "Triangulaire", "Spécifiques (AT/REC)", "NESDA / Rappels"])

with tabs[0]: st.session_state.v.add("1"); build_rubrique("1. Achat Matière Première", "MP")
with tabs[1]: st.session_state.v.add("2"); build_rubrique("2. Formule Triangulaire", "TRI", True)
with tabs[2]:
    st.session_state.v.add("3")
    build_rubrique("5. Algérie Télécom", "AT", True)
    build_rubrique("6. Recyclage", "REC", True)
with tabs[3]:
...
if len(st.session_state.v) >= 4:
    if st.button("💾 ENREGISTRER & GÉNÉRER PDF", type="primary", use_container_width=True):
        if save_to_gsheet(data):
            ...
            st.download_button("📥 TÉLÉCHARGER LE PDF", data=pdf_out, file_name=f"Bilan_{mois}.pdf", mime="application/pdf")
            st.balloons()
else:
    st.warning(f"Veuillez parcourir les 4 onglets ({len(st.session_state.v)}/4)")
